         ...
         <div class="modal-content">
            <form id="assignedToForm">
               ...
               <div class="modal-header">
                <h5 class="modal-title">Re-assign Ticket To</h5>
                  <button type="button" class="close btn btn-link p-0" data-dismiss="modal">&times;</button>
               </div>
               <div class="modal-body py-2">               
                  <div class="form-group mb-2">
                     <label><?php echo $_SESSION['tier']==1?'Lead Name':'Staff Name'; ?> <span class="text-danger">*</span></label>
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              <div class="row mb-5">
                  <div id="table-list-container" class="col-md-12">
                    <!-- Striped Rows -->
                    <div class="card">
                      <h5 class="card-header pb-3"><?php echo "<i class='bx bxs-package'></i> For Review Tickets"?>
                          ...
                          <div class="ticket-thread-header clearfix">
                            <div class="row">
                              ...
                              <div class="col-lg-6 hidden-sm text-right">
                                <a href="javascript:void(0);" class="btn btn-outline-secondary btn-sm" id="btn-update-assign-to"><i class="bx bx-group"></i> Re-assign</a>
                                <a href="javascript:void(0);" class="btn btn-outline-success btn-sm doneBtn"><i class="bx bxs-user-check"></i> Done</a>
            ...
            <script type="text/javascript">              
              $(document).ready(function(){                
                function fetchData() {
                    ...
                    $.post("/ticketing/fetch-tickets/",{"status":"internal review","token":localStorage.getItem("token")},function(data){
                ...
                $(document).on('click', '#btn-update-assign-to', function() {
                  var login_tier = <?php echo $_SESSION['tier']; ?>;
                  var tier = 3;
                  var tier_txt = "Select Staff";
                  if(login_tier==1){
                    tier = 2;
                    var tier_txt = "Select Lead";
                  }
                  ...
                  $.post("/ticketing/fetch_ticketing_user",{"token":localStorage.getItem("token"),"tier": tier,"site_id":site_id},function(response){
                    ...
                    if (result.status) {
                      ...
                      console.log(result.data);
                        ...
                        var users = '<option value="">'+tier_txt+'</option>';
                ...
                $(".doneBtn").click(function(){
                  ...
                  var tier = <?php echo $_SESSION['tier']; ?>;
                  if(tier==1){
                    toastr.success('<button type="button" class="btn btn-primary btn-toastr btn-toastr-yes-done mt-2">YES</button><button type="button" class="btn btn-outline-secondary btn-toastr btn-toastr-no mt-2">NO</button>' , 'Are you sure you want to close the ticket?');
                  } else {
                ...
                $(document).on('click', '.btn-toastr-yes-done', function() { 
                  var tier = <?php echo $_SESSION['tier']; ?>;
                  var ticket_status = tier==1?'closed':'internal review';               
                  $.post("/ticketing/update-ticket-status",{"token":localStorage.getItem("token"),"t":"tickets","ticket_id":$("#ticket_id").val(), "status":ticket_status},function(response){
                    var result = JSON.parse(response);
                    if (result.status) {
                        toastr.success(result.message, 'Good Job!')          
                        if (screenWidth >= 992) {
