            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              <div class="row mb-5">
                  <div id="table-list-container" class="col-md-12">
                    <!-- Striped Rows -->
                    <div class="card">
                      <h5 class="card-header pb-3"><?php echo "<i class='bx bxs-package'></i> Completed Tickets"?>
                          ...
                          <div class="ticket-thread-header clearfix">
                            <div class="row">
                              ...
                              <div class="col-lg-6 hidden-sm text-right">
                                <?php 
                                  if($_SESSION['tier'] == 2){
                                    echo '<a href="javascript:void(0);" class="btn btn-outline-secondary btn-sm" id="btn-update-assign-to"><i class="bx bx-group"></i> Re-assign</a> ';
            ...
            <script type="text/javascript">              
              $(document).ready(function(){                
                function fetchData() {
                    ...
                    $.post("/ticketing/fetch-tickets/",{"status":"closed","token":localStorage.getItem("token")},function(data){
                ...
                $(document).on('click', '.btn-toastr-yes-done', function() {                  
                  $.post("/ticketing/update-ticket-status",{"token":localStorage.getItem("token"),"t":"tickets","ticket_id":$("#ticket_id").val(), "status":"client review"},function(response){
                    var result = JSON.parse(response);
                    if (result.status) {
                        toastr.success(result.message, 'Good Job!')          
                        if (screenWidth >= 992) {
