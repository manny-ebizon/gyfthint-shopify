            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              <div class="row mb-5">
                  <div id="table-list-container" class="col-md-12">
                    <!-- Striped Rows -->
                    <div class="card">
                      <h5 class="card-header pb-3"><?php echo "<i class='bx ".$data['roleaccess']['module_icon']."'></i> Raised Tickets"?>
                          ...
                          <div class="ticket-thread-header clearfix">
                            <div class="row">
                              ...
                              <div class="col-lg-6 hidden-sm text-right">
                                  <a href="javascript:void(0);" class="btn btn-outline-secondary btn-sm" id="btn-update-assign-to"><i class='bx bx-group'></i> Assign</a>
                                  <a href="javascript:void(0);" class="btn btn-outline-success btn-sm"><i class='bx bxs-user-check'></i> Done</a>
            ...
            <script type="text/javascript">              
              $(document).ready(function(){                
                function fetchData() {
                    ...
                    $.post("/ticketing/fetch-tickets/",{"status":"raised","token":localStorage.getItem("token")},function(data){
                  ...
                  $.post("/ticketing/fetch_ticketing_user",{"token":localStorage.getItem("token"),"tier": tier,"site_id":site_id},function(response){
                    ...
                    if (result.status) {
                      ...
                      console.log(result.data);
                        ...
                        var users = '<option value="">Select Lead</option>';
                ...
                $("#assignedToForm").submit(function(e){
                  ...
                  $("button[type=submit]",$this).attr("disabled","disabled");                
                  $("button[type=submit]",$this).html(loading_text);
                  $.post("/ticketing/update_assignee",{"token":localStorage.getItem("token"),"t":"ticket_management_relation","data":$(this).serialize(), "status":"assigned"},function(response){
                    $("button[type=submit]",$this).html(button_text);
                    $("button[type=submit]",$this).removeAttr("disabled");
                    var result = JSON.parse(response);
                    if (result.status) {
