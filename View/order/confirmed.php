                  ...
                  <div class="tab-pane fade show active" id="navs-pills-blogger-outreach-service" role="tabpanel">
                    <div class="row">
                      <div id="table-list-container" class="col-md-12">
                        <!-- Striped Rows -->
                        <div class="card">
                          <h5 class="card-header pb-3"><i class='bx bxs-box me-1'></i> Confirmed Orders                        
                          </h5>                      
              ...
              $(document).ready(function(){                
                function fetchData() {
                  var service = "all";
                    ...
                    $.post("/api/fetch-confirmed-orders/",{"service":service,"token":localStorage.getItem("token")},function(data){
                ...
                $("#blogger-datatable").on("click",".approvedBtn",function(){
                  var id = $(this).data("id");
                  $.post("/api/set-order-status",{"token":localStorage.getItem("token"),"order_id":id,"status":"Confirmed Order"},function(response){
                    var result = JSON.parse(response);
                      if (result.status) {
                ...
                $("#blogger-datatable").on("click",".completedBtn",function(){
                  var id = $(this).data("id");
                  $(this).attr("disabled","disabled");
                  $(this).html(loading_text);
                  $.post("/api/set-order-status",{"token":localStorage.getItem("token"),"order_id":id,"status":"Completed"},function(response){
                    var result = JSON.parse(response);
                      if (result.status) {
