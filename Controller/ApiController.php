...
function register_customer()
{
    if (isset($_POST['data'])) {
        ...
        if ($res === true) {
            ...
            $results = getShortlink($return['redirect_url']);
            if (isset($results['success']) && $results['success']) {
                $return['redirect_url'] = $results['data'];
            } else {
                $return['status'] = false;
...
function getShortlink($redirectUrl): array
{
    $url = 'https://firebasedynamiclinks.googleapis.com/v1/shortLinks?key=' . FIREBASE_DYNAMIC_LINK;
    $data = [
        'dynamicLinkInfo' => [
            'domainUriPrefix' => 'https://gyfthintproduction.page.link',
            'link' => $redirectUrl,
            'androidInfo' => [
                'androidPackageName' => FIREBASE_DYNAMIC_LINK_ANDROID_PACKAGE_NAME
            ],
            'iosInfo' => [
                'iosBundleId' => FIREBASE_DYNAMIC_LINK_IOS_BUNDLE_ID,
                'iosAppStoreId' => FIREBASE_DYNAMIC_LINK_IOS_APPSTORE_ID
            ],
        ],
    ];
